  ...
  methods: {
    ...
    mouseUp: function () {
      ...
      if (this.dragondData.selectText.length > 0) {
        ...
        const posX = window.getSelection().getRangeAt(0).getBoundingClientRect().x + (window.getSelection().getRangeAt(0).getBoundingClientRect().right - window.getSelection().getRangeAt(0).getBoundingClientRect().left) / 2;
